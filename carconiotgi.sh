...
while [[ true ]]; do
	...
	if [[ $(iw dev $int link | grep SSID) != "" ]] && [[ $flag != "c1" ]]; then
		...
		#Start transmission to server E - GI
		hping3 --udp -p 5002 -i u48000 -d 1470 10.0.2.11 -q &
		h2=$!
		ping 10.0.2.11 -i 1 -c 330 | while read line; do echo $(date +%s) - $line >> ping$sta\_h2.txt; done &
        p2=$!
